...
def fit_tree(input_frame, test_frac = 0.1):
    ...
    joblib.dump(time_fit, '../data/gb_time_fit.sav')
    amplitude_fit.fit(X_train, Y_train[:,1])
    joblib.dump(amplitude_fit, '../data/gb_amplitude_fit.sav')
...
if __name__ == '__main__':
    ...
    tree_data = pd.read_json('../data/training_data_2000000.json')
    # Get the model
    (time_fit, amplitude_fit) = fit_tree(tree_data)
    end = timer()
    print('Gradient boosting training time: {0}'.format(end - start))
