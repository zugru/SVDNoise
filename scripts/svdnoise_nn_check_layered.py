...
def plot_density(threshold, sigma, width, n_random_samples = 10000):
    ...
    recX = np.empty((n_random_samples, 4))
    recX[:,0] = sigma
    recX[:,1] = width
    recX[:,2] = u
    recX[:,3] = v
...
if __name__=='__main__':
    ...
    for threshold, sigma, width in triples:
        # %% Load the model
        model_t0 = joblib.load('../data/nn_model_t0_thr{0}.sav'.format(int(threshold)))
        model_amplitude = joblib.load('../data/nn_model_amplitude_thr{0}.sav'.format(int(threshold)))
        # %% Make the plot
        print('Plotting maps for thr{0} sig{1} w{2}:'.format(threshold, sigma, width))
        plot_diff_map(threshold, sigma, width)
        plot_density(threshold, sigma, width, 10000)
