...
def plot_weights(bgm, threshold):
    fig = plt.figure(figsize=(10,10))
    plt.scatter(range(len(bgm.weights_)), np.flip(np.sort(bgm.weights_), axis = 0))
    plt.savefig('{0}/bgm_weights_thr{1}.png'.format(plotdir, threshold))
...
def plot_margin_densities(bgm_fit, threshold):
    ...
    ax1.plot(u, m_sigma)
    ax2.plot(u, m_width)
    plt.savefig('{0}/margin_densities_thr{1}.png'.format(plotdir, threshold))


def make_conditional_density(bgm_fit, threshold, sigma, width):
    """ Compute weights, means and covariances for density with given 
    sigma and width"""
    pass
...
if __name__ == '__main__':
    ...
    # %% Load the model
        bgm_fit = joblib.load('../data/bgm_model_thr{0}.sav'.format(int(threshold)))
        print('Plotting maps for threshold {0}...'.format(threshold))
        plot_weights(bgm_fit.named_steps['bayesiangaussianmixture'], threshold)
        plot_margin_densities(bgm_fit, threshold)
